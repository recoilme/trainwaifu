...
class TextEmbeddingCache(WebhookMixin):
    ...
    def compute_waifu_prompt(self, prompt: str):
        print("compute_waifu_prompt",prompt)
        text_encoder_model_max_length = 512
        txt_tokens = self.tokenizers[0](prompt, return_tensors="pt", padding="max_length", max_length = text_encoder_model_max_length ,truncation=True).to("cuda")
        select_index = [0] + list(
            range(-text_encoder_model_max_length + 1, 0)
        )
        #prompt_embeds = self.text_encoders[0](txt_tokens.input_ids, attention_mask=txt_tokens.attention_mask, output_hidden_states=False, output_attentions=False).last_hidden_state[:, None][
        #    :, :, select_index
        #    ]
        prompt_embeds = self.text_encoders[0](txt_tokens.input_ids, attention_mask=txt_tokens.attention_mask).last_hidden_state[:, None][
            :, :, select_index
            ]
                
        prompt_attention_mask = txt_tokens.attention_mask[:, None, None][:, :, :, select_index]
    ...
    def compute_embeddings_for_prompts(
        ...
        elif self.model_type == "waifu":
            output = self.compute_embeddings_for_waifu_prompts(
                raw_prompts,
                return_concat=return_concat,
                load_from_cache=load_from_cache,
            )
    ...
    def compute_embeddings_for_waifu_prompts(
        self,
        prompts: list = None,
        return_concat: bool = True,
        load_from_cache: bool = True,
        ...
        with torch.no_grad():
            ...
            for prompt in tqdm(
                ...
                if should_encode:
                    self.debug_log(f"Encoding waifu prompt: {prompt}")
                    # Get the current size of the queue.
                    current_size = self.write_queue.qsize()
                    if current_size >= 2048:
                        log_msg = str(
                        ...
                        while self.write_queue.qsize() > 100:
                            ...
                            time.sleep(5)
                    # TODO: Batch this
                    prompt_embeds, attention_mask = self.compute_waifu_prompt(
                        prompt=prompt,
                    )
                    if "deepfloyd" not in StateTracker.get_args().model_type:
                        # we have to store the attn mask with the embed for pixart.
